...
export function Signup() {
   ...
   const callLogin=async(e)=>{
    e.preventDefault();
     await axios.post("https://e-commerce-backend-4908.onrender.com/user/login",{
        email:email,
        password:password
     }).then(res=>{
    ...
    return (
        <>
             ...
             <div className='text-white p-4 w-full bg-amber-400'>Costumer Login</div>
            ...
            <div className=' flex items-center justify-center'><button className='bg-green-400 rounded-3xl p-4 border-2 border-amber-300 m-5 ' type='submit'>SUBMIT</button></div>
             ...
             <Link to='/sellerlogin'>
              <button className='text-white p-4 w-55 bg-red-500 rounded-full inline cursor-pointer m-4'>LogIn As Seller</button>
              </Link>
            </div>
